import { loadHeaderFooter, renderListWithTemplate, isValidRelationshipAdvice } from "./utils.mjs";
...
async function getExternalEmergencyAdvice(retryCount = 0) {
  ...
  if (retryCount >= 5) {
    console.log("Max retries reached. Using high-quality backup relationship advice.");
    ...
    const randomBackupText = highQualityBackups[Math.floor(Math.random() * highQualityBackups.length)];
  ...
  try {
    ...
    if (!isValidRelationshipAdvice(rawText)) {
      console.log(`[Attempt ${retryCount + 1}] Filtered out: "${rawText}". Retrying...`);
    ...
    renderCleanAdviceCard(container, { id: 0, text: "Take a deep breath. Clear communication is key." });
  }
}
...
// Set up the interval loop to run every 3 seconds
setInterval(() => {
  getExternalEmergencyAdvice();
}, 12000);